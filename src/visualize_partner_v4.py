...
def _h5_to_python_flat(f):
    """
    Load top-level H5 datasets/groups into a plain Python dict.
    This is enough for the file style you showed:
      point_type, temperature, times, u, v, xy_norm, xy_raw
    """
    out = {}
    for k in f.keys():
        obj = f[k]
        if isinstance(obj, h5py.Dataset):
            ...
            if np.isscalar(val):
                ...
                out[k] = _decode_if_bytes(val)
            else:
                out[k] = _decode_if_bytes(val).tolist()
            ...
            for kk in obj.keys():
                ...
                if np.isscalar(vv):
                    ...
                    sub[kk] = _decode_if_bytes(vv)
                else:
                    sub[kk] = _decode_if_bytes(vv).tolist()
            out[k] = sub
    return out
...
def load_structured_file(path: str):
    ...
    Returns a normal Python object.
    """
    p = Path(path)
    ...
    if suffix in {".h5", ".hdf5"}:
        if h5py is None:
            raise ImportError("h5py is required to read .h5/.hdf5 files. Install with: pip install h5py")
        with h5py.File(p, "r") as f:
            return _h5_to_python_flat(f)

    raise ValueError(f"Unsupported file type '{suffix}'. Expected .json, .h5, or .hdf5")
...
def load_geom(geom_path: str):
    ...
    Returns:
      ...
      obj: full object
    """
    obj = load_structured_file(geom_path)
    W = int(obj["width"])
    H = int(obj["height"])
...
def detect_pred_format(obj):
    ...
    Supported formats:
      ...
      5) flat H5-style output:
         {
           "xy_raw":[[x,y], ...] or "xy_norm":[[x,y], ...],
           "times":[...],                # optional for steady fields
           "temperature":[T,N] or [N],
           "u":[N],
           "v":[N],
           "p":[N],
           "point_type":[N]
    ...
    if isinstance(obj, dict):
        ...
        has_xy_like = ("xy" in obj) or ("xy_raw" in obj) or ("xy_norm" in obj)
        has_scalar_field = any(k in obj for k in ["values", "temperature", "T", "u", "v", "p"])
        if has_xy_like and has_scalar_field:
            return "xy_like_top_level"

    raise ValueError(
        "Unknown pred format. Expected one of: flat-list, packed-points, xy/times/values, xy/fields steady, or top-level xy-like H5 format."
    )
...
def detect_scalar_field_name_from_xy_times_values(obj):
    ...
    skip = {"xy", "times", "norm", "point_type", "points", "timesteps", "xy_raw", "xy_norm"}
...
def detect_scalar_field_name_from_xy_like_top_level(obj):
    priority = ["temperature", "T", "u", "v", "p", "values"]
    for k in priority:
        if k in obj:
            return k

    skip = {"xy", "xy_raw", "xy_norm", "times", "point_type", "norm", "points", "timesteps"}
    for k, v in obj.items():
        if k in skip:
            continue
        if isinstance(v, list):
    ...
    raise ValueError(f"Could not detect scalar field in top-level xy-like object keys: {list(obj.keys())}")
...
def load_pred(pred_path: str, field_name: str = None):
    ...
    """
    obj = load_structured_file(pred_path)
    ...
    # -------------------------
    # top-level xy-like H5 format
    # -------------------------
    if fmt == "xy_like_top_level":
        if "xy_raw" in obj and obj["xy_raw"] is not None:
            xy_use = np.asarray(obj["xy_raw"], dtype=np.float32)
        elif "xy" in obj and obj["xy"] is not None:
            xy_use = np.asarray(obj["xy"], dtype=np.float32)
        elif "xy_norm" in obj and obj["xy_norm"] is not None:
            xy_use = np.asarray(obj["xy_norm"], dtype=np.float32)
        else:
            raise ValueError("Top-level xy-like format requires one of: xy_raw, xy, xy_norm")

        scalar_name = field_name or detect_scalar_field_name_from_xy_like_top_level(obj)

        if scalar_name not in obj:
            raise ValueError(f"Requested field '{scalar_name}' not found in keys={list(obj.keys())}")
        ...
        times_raw = obj.get("times", None)

        if vals.ndim == 1:
            t = 0.0
            if vals.shape[0] != xy_use.shape[0]:
                raise ValueError(
                    f"Mismatch: steady field '{scalar_name}' has N={vals.shape[0]} but xy has N={xy_use.shape[0]}"
                )
            ...
            if times_raw is None:
                times = [float(i) for i in range(vals.shape[0])]
            else:
                times = [float(t) for t in times_raw]

            if vals.shape[0] != len(times):
                raise ValueError(
                    f"Mismatch: field '{scalar_name}' has T={vals.shape[0]} but times has {len(times)}"
                )

            if vals.shape[1] != xy_use.shape[0]:
                raise ValueError(
                    f"Mismatch: field '{scalar_name}' has N={vals.shape[1]} but xy has N={xy_use.shape[0]}"
                )
            ...
            raise ValueError(
                f"Expected top-level field '{scalar_name}' to have shape [N] or [T,N], got {vals.shape}"
            )
...
def main():
    ...
    ap.add_argument("--geom", required=False, default="./data/partner_v4/pipe_three_class_fixed.json",
                    help="Geometry file (.json/.h5/.hdf5) with width/height/points/type.")
    ap.add_argument("--pred", required=False, default="./data/partner_v4/pipe_three_class_fixed_pred_flow_steady.json",
                    help="Prediction file (.json/.h5/.hdf5). Supports flat-list, packed-points, xy/times, steady xy/fields, and top-level H5 layout.")
